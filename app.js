    ...
    doctors?.forEach((doctor) => {
        ...
        div.innerHTML = `
            ...
            <p class="text-center font-semibold mt-2 mb-4">${doctor?.specialization?.map((item) => {
                return `<button class="bg-teal-600 text-white py-2 px-4 rounded-lg text-lg transition-all duration-300 ease-in-out hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500">${item}</button>`;
            })}</p>
